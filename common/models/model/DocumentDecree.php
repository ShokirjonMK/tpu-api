 ...
 */
class DocumentDecree extends \yii\db\ActiveRecord
{
    ...
    const UPLOADS_FOLDER = 'uploads/document/decree/';
    ...
    {
        return $this->hasOne(DocumentDecreeBody::className(), ['document_decree_id' => 'id'])->where(['status' => 1, 'is_deleted' => 0]);
    }

    public function getInfo()
    {
        return $this->hasMany(DocumentDecreeInfo::className(), ['document_decree_id' => 'id']);
    }
    ...
    public static function createItem($model, $post)
    {
        ...
        $model->message = null;
        if ($model->save(false)) {
            $model->ids = 'doc-d-'.$model->id;
            $model->save(false);
            $body = new DocumentDecreeBody();
            $body->document_decree_id = $model->id;
