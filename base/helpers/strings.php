...
function profileFullName($profile = null)
{
    ...
    if ($profile->user->position == null) {
        $postition = $profile->user->attach_role;
    } else {
...
function examConfirm($model)
{
    $profile = current_user_profile();

    $year = $model->eduYear;

    $nomer = '№:' . $year->start_year.'-'.$year->end_year.'/'.$model->semestr_id.'-'.$model->id;

    $name = getFirstName($profile->first_name);
    $mid_name = getFirstName($profile->middle_name);
    return $name.".".$mid_name.".".$profile->last_name.PHP_EOL.$nomer.PHP_EOL.date("Y-m-d H:i:s");
}
