...
class DocumentTypeController extends ApiActiveController
{
    ...
    public $table_name = 'document_type';
    public $controller_name = 'DocumentType';
    ...
    public function actionCreate($lang)
    {
        ...
        $result = DocumentType::createItem($model, $post);
        if (!is_array($result)) {
    ...
    {
        $model = DocumentType::findOne($id);
        if (!$model) {
            return $this->response(0, _e('Data not found.'), null, null, ResponseStatus::NOT_FOUND);
        }
        $post = Yii::$app->request->post();
        $this->load($model, $post);
        $result = DocumentType::updateItem($model, $post);
        if (!is_array($result)) {
