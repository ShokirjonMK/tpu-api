...
class User extends CommonUser
{
    ...
    public function createUser($user, $profile)
    {
        ...
        if ($user->save()) {
            ...
            // Create model according to selected Role 

            if(in_array($user->roleName, EmployeeUser::$roleList)){
                $employee = new Employee();
                $employee->user_id = $user->id;
                $employee->save();
            };

            if(in_array($user->roleName, StudentUser::$roleList)){
                $student = new Student();
    ...
    public function updateUser($user, $profile, $post_item)
    {
        ...
        if ($user->save(false) && $profile->save()) {
            ...
            if(in_array($user->roleName, EmployeeUser::$roleList)){
                // Agar student object bolsa o'chiramiz
                $student = Student::findOne(['user_id' => $user->id]);
                if ($student) $student->delete();
                // Agar employee object bolmasa yangi yaratamiz
            ...
            if(in_array($user->roleName, StudentUser::$roleList)){
                // Agar employee object bolsa o'chiramiz
                $employee = Employee::findOne(['user_id' => $user->id]);
                if ($employee) $employee->delete();

                // Agar student object bolmasa yangi yaratamiz
                $student = Student::findOne(['user_id' => $user->id]);
                if(!$student){
                    $student = new Student();
                    $student->user_id = $user->id;
