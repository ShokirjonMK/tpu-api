...
class Subject extends CommonSubject
{
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        if (is_numeric($department) && $department > 0) {
            $query->andWhere(['subject.department_id' => $department]);
        }
        ...
        if ($sort == 'a-z') {
            ...
            $sort_query = ['subject.created_on' => SORT_ASC];
        } else {
            $sort_query = ['subject.created_on' => SORT_DESC];
        }
    ...
    public static function getPageTypes($active_key = '')
    {
        $page_types = array(
            'index' => array(
                'name' => _e('All'),
                'active' => false,
                'count' => self::itemsCount(['subject.deleted' => 0]),
            ),
            'published' => array(
                'name' => _e('Published'),
                'active' => false,
                'count' => self::itemsCount(['subject.deleted' => 0, 'subject.status' => 1]),
            ),
            'unpublished' => array(
                'name' => _e('Unpublished'),
                'active' => false,
                'count' => self::itemsCount(['subject.deleted' => 0, 'subject.status' => 0]),
            ),
            'deleted' => array(
                'name' => _e('Deleted'),
                'active' => false,
                'count' => self::itemsCount(['subject.deleted' => 1]),
            ),
        );
    ...
    public static function createItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['subject']['attrs'] = $model->getAttributes();
            $log_data['subject']['old_attrs'] = array();
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $new = new SubjectInfo();
                    $new->subject_id = $model->id;
                    $new->language = $lang_code;
                    $new->name = $info->name[$lang_code];
                    $new->description = $info->description[$lang_code];

                    if ($new->save()) {
                        $log_data['subject_info'][$new->language]['attrs'] = $new->getAttributes();
                        $log_data['subject_info'][$new->language]['old_attrs'] = array();
                    }else{
                        dd($new->errors);
                    }
                }
    ...
    public static function updateItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['subject']['attrs'] = $model->getAttributes();
            $log_data['subject']['old_attrs'] = $modelOldAttributes;
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $infoSingleLang = SubjectInfo::find()->where(['subject_id' => $model->id, 'language' => $lang_code])->one();
                    if($infoSingleLang){
                        ...
                        if ($infoSingleLang->save()) {
                            $log_data['subject_info'][$infoSingleLang->language]['attrs'] = $infoSingleLang->getAttributes();
                            $log_data['subject_info'][$infoSingleLang->language]['old_attrs'] = $infoOldAttributes;
                        }else{
    ...
    public static function getItemToEdit($id)
    {
        ...
        $translations = SubjectInfo::find()->where(['subject_id' => $id])->all();
        $names = [];
        $descriptions = [];
    ...
    public static function ajaxAction($action, $id, $items)
    {
        ...
        if ($action == 'unpublish') {
            ...
            if (is_numeric($id) && $id > 0) {
                ...
                self::bulkActionWithSubject('unpublish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSubject('publish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSubject('trash', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSubject('restore', $item);
     ...
     * @return void
     */
    public static function bulkActionWithSubject($type, $model, $with_childs = true)
    {
        if ($model) {
     ...
     * @return void
     */
    public static function deleteSubject($model, $with_childs = true)
    {
        if ($model) {
            ...
            if ($model->delete(false)) {
                $id = $model->id;
                $info = SubjectInfo::find()->where(['content_id' => $id])->all();
