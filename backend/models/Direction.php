...
class Direction extends CommonDirection
{
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        if (is_numeric($department) && $department > 0) {
            $query->andWhere(['direction.department_id' => $department]);
        }
        ...
        if ($sort == 'a-z') {
            ...
            $sort_query = ['direction.created_on' => SORT_ASC];
        } else {
            $sort_query = ['direction.created_on' => SORT_DESC];
        }
    ...
    public static function getPageTypes($active_key = '')
    {
        $page_types = array(
            'index' => array(
                'name' => _e('All'),
                'active' => false,
                'count' => self::itemsCount(['direction.deleted' => 0]),
            ),
            'published' => array(
                'name' => _e('Published'),
                'active' => false,
                'count' => self::itemsCount(['direction.deleted' => 0, 'direction.status' => 1]),
            ),
            'unpublished' => array(
                'name' => _e('Unpublished'),
                'active' => false,
                'count' => self::itemsCount(['direction.deleted' => 0, 'direction.status' => 0]),
            ),
            'deleted' => array(
                'name' => _e('Deleted'),
                'active' => false,
                'count' => self::itemsCount(['direction.deleted' => 1]),
            ),
        );
    ...
    public static function createItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['direction']['attrs'] = $model->getAttributes();
            $log_data['direction']['old_attrs'] = array();
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $new = new DirectionInfo();
                    $new->direction_id = $model->id;
                    $new->language = $lang_code;
                    $new->name = $info->name[$lang_code];
                    $new->description = $info->description[$lang_code];

                    if ($new->save()) {
                        $log_data['direction_info'][$new->language]['attrs'] = $new->getAttributes();
                        $log_data['direction_info'][$new->language]['old_attrs'] = array();
                    }else{
                        dd($new->errors);
                    }
                }
    ...
    public static function updateItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['direction']['attrs'] = $model->getAttributes();
            $log_data['direction']['old_attrs'] = $modelOldAttributes;
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $infoSingleLang = DirectionInfo::find()->where(['direction_id' => $model->id, 'language' => $lang_code])->one();
                    if($infoSingleLang){
                        ...
                        if ($infoSingleLang->save()) {
                            $log_data['direction_info'][$infoSingleLang->language]['attrs'] = $infoSingleLang->getAttributes();
                            $log_data['direction_info'][$infoSingleLang->language]['old_attrs'] = $infoOldAttributes;
                        }else{
    ...
    public static function getItemToEdit($id)
    {
        ...
        $translations = DirectionInfo::find()->where(['direction_id' => $id])->all();
        $names = [];
        $descriptions = [];
    ...
    public static function itemsCount($where = array(), $where_in = array())
    {
        ...
        $query->groupBy('info.direction_id');
        return $query->count();
    }
    ...
    public static function ajaxAction($action, $id, $items)
    {
        ...
        if ($action == 'unpublish') {
            ...
            if (is_numeric($id) && $id > 0) {
                ...
                self::bulkActionWithDirection('unpublish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithDirection('publish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithDirection('trash', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithDirection('restore', $item);
     ...
     * @return void
     */
    public static function bulkActionWithDirection($type, $model, $with_childs = true)
    {
        if ($model) {
     ...
     * @return void
     */
    public static function deleteDirection($model, $with_childs = true)
    {
        if ($model) {
            ...
            if ($model->delete(false)) {
                $id = $model->id;
                $info = DirectionInfo::find()->where(['content_id' => $id])->all();
