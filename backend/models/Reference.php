...
class Reference extends CommonReference
{
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        if ($sort == 'a-z') {
            ...
            $sort_query = ['reference.created_on' => SORT_ASC];
        } else {
            $sort_query = ['reference.created_on' => SORT_DESC];
        }
     ...
     * @return array
     */
    public static function getPageTypes($type, $active_key = '')
    {
        $page_types = array(
            'all' => array(
                'name' => _e('All'),
                'active' => false,
                'count' => self::itemsCount(['reference.deleted' => 0,'type' => $type]),
            ),
            'published' => array(
                'name' => _e('Published'),
                'active' => false,
                'count' => self::itemsCount(['reference.deleted' => 0, 'reference.status' => 1,'type' => $type]),
            ),
            'unpublished' => array(
                'name' => _e('Unpublished'),
                'active' => false,
                'count' => self::itemsCount(['reference.deleted' => 0, 'reference.status' => 0,'type' => $type]),
            ),
            'deleted' => array(
                'name' => _e('Deleted'),
                'active' => false,
                'count' => self::itemsCount(['reference.deleted' => 1,'type' => $type]),
            ),
        );
    ...
    public static function createItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['reference']['attrs'] = $model->getAttributes();
            $log_data['reference']['old_attrs'] = array();
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $new = new ReferenceInfo();
                    $new->reference_id = $model->id;
                    $new->language = $lang_code;
                    $new->name = $info->name[$lang_code];
                    $new->description = $info->description[$lang_code];

                    if ($new->save()) {
                        $log_data['reference_info'][$new->language]['attrs'] = $new->getAttributes();
                        $log_data['reference_info'][$new->language]['old_attrs'] = array();
                    }else{
                        dd($new->errors);
                    }
                }
    ...
    public static function updateItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['reference']['attrs'] = $model->getAttributes();
            $log_data['reference']['old_attrs'] = $modelOldAttributes;
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $infoSingleLang = ReferenceInfo::find()->where(['reference_id' => $model->id, 'language' => $lang_code])->one();
                    if($infoSingleLang){
                        ...
                        if ($infoSingleLang->save()) {
                            $log_data['reference_info'][$infoSingleLang->language]['attrs'] = $infoSingleLang->getAttributes();
                            $log_data['reference_info'][$infoSingleLang->language]['old_attrs'] = $infoOldAttributes;
                        }else{
    ...
    public static function getItemToEdit($id)
    {
        ...
        $translations = ReferenceInfo::find()->where(['reference_id' => $id])->all();
        $names = [];
        $descriptions = [];
    ...
    public static function itemsCount($where = array(), $where_in = array())
    {
        ...
        $query->groupBy('info.reference_id');
        return $query->count();
    }
    ...
    public static function ajaxAction($action, $id, $items)
    {
        ...
        if ($action == 'unpublish') {
            ...
            if (is_numeric($id) && $id > 0) {
                ...
                self::bulkActionWithReference('unpublish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithReference('publish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithReference('trash', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithReference('restore', $item);
     ...
     * @return void
     */
    public static function bulkActionWithReference($type, $model, $with_childs = true)
    {
        if ($model) {
     ...
     * @return void
     */
    public static function deleteReference($model, $with_childs = true)
    {
        if ($model) {
            ...
            if ($model->delete(false)) {
                $id = $model->id;
                $info = ReferenceInfo::find()->where(['content_id' => $id])->all();
