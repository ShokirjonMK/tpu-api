 ...
 */
class StudentUser extends CommonUser
{
    const ACTIVE = 10;
    const BANNED = 5;
    const PENDING = 0;
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        $query = self::find()
            ->with(['profile','student.department.infoRelation'])
            ->leftJoin('profile', 'profile.user_id = users.id')
            ->leftJoin('student', 'student.user_id = users.id')
            ...
            $query->andWhere(['student.department_id' => $department]);
        }
     ...
     * @return int
     */
    public function createUser($user, $profile, $student)
    {
        $now = time();
        $log_data = array();
        ...
        if ($user->save()) {
            ...
            if ($student->save()) {
                $log_data['student']['attrs'] = $student->getAttributes();
                $log_data['student']['old_attrs'] = array();
            }else{
                dd($student->errors);
            }
     ...
     * @return int
     */
    public function updateUser($user, $profile, $student, $post_item)
    {
        $now = time();
        $log_data = array();
        ...
        $student_oldAttributes = $student->getOldAttributes();

        if ($user->save(false) && $profile->save() && $student->save()) {
            $log_data['user']['attrs'] = $user->getAttributes();
            ...
            $log_data['student']['attrs'] = $student->getAttributes();
            $log_data['student']['old_attrs'] = $student_oldAttributes;
