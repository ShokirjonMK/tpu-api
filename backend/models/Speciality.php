...
use common\models\SpecialityInfo;
use common\models\Speciality as CommonSpeciality;

/**
 * Speciality model
 */
class Speciality extends CommonSpeciality
{
    public static $type;
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        $query = self::find()
            ->join('INNER JOIN', 'speciality_info info', 'info.speciality_id = speciality.id');
        ...
        if ($sort == 'a-z') {
            ...
            $sort_query = ['speciality.created_on' => SORT_ASC];
        } else {
            $sort_query = ['speciality.created_on' => SORT_DESC];
        }
    ...
    public static function getPageTypes($active_key = '')
    {
        $page_types = array(
            'index' => array(
                'name' => _e('All'),
                'active' => false,
                'count' => self::itemsCount(['speciality.deleted' => 0]),
            ),
            'published' => array(
                'name' => _e('Published'),
                'active' => false,
                'count' => self::itemsCount(['speciality.deleted' => 0, 'speciality.status' => 1]),
            ),
            'unpublished' => array(
                'name' => _e('Unpublished'),
                'active' => false,
                'count' => self::itemsCount(['speciality.deleted' => 0, 'speciality.status' => 0]),
            ),
            'deleted' => array(
                'name' => _e('Deleted'),
                'active' => false,
                'count' => self::itemsCount(['speciality.deleted' => 1]),
            ),
        );
    ...
    public static function createItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['speciality']['attrs'] = $model->getAttributes();
            $log_data['speciality']['old_attrs'] = array();
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $new = new SpecialityInfo();
                    $new->speciality_id = $model->id;
                    $new->language = $lang_code;
                    $new->name = $info->name[$lang_code];
                    $new->description = $info->description[$lang_code];

                    if ($new->save()) {
                        $log_data['speciality_info'][$new->language]['attrs'] = $new->getAttributes();
                        $log_data['speciality_info'][$new->language]['old_attrs'] = array();
                    }else{
                        dd($new->errors);
                    }
                }
    ...
    public static function updateItem($model, $info, $post_item = array())
    {
        ...
        if ($model->save()) {
            $log_data['speciality']['attrs'] = $model->getAttributes();
            $log_data['speciality']['old_attrs'] = $modelOldAttributes;
            ...
            if ($active_languages) {
                ...
                foreach ($active_languages as $active_language) {
                    ...
                    $infoSingleLang = SpecialityInfo::find()->where(['speciality_id' => $model->id, 'language' => $lang_code])->one();
                    if($infoSingleLang){
                        ...
                        if ($infoSingleLang->save()) {
                            $log_data['speciality_info'][$infoSingleLang->language]['attrs'] = $infoSingleLang->getAttributes();
                            $log_data['speciality_info'][$infoSingleLang->language]['old_attrs'] = $infoOldAttributes;
                        }else{
    ...
    public static function getItemToEdit($id)
    {
        ...
        $info = new SpecialityInfo();
        $translations = SpecialityInfo::find()->where(['speciality_id' => $id])->all();
        $names = [];
        $descriptions = [];
    ...
    public static function itemsCount($where = array(), $where_in = array())
    {
        ...
        $query->groupBy('info.speciality_id');
        return $query->count();
    }
    ...
    public static function ajaxAction($action, $id, $items)
    {
        ...
        if ($action == 'unpublish') {
            ...
            if (is_numeric($id) && $id > 0) {
                ...
                self::bulkActionWithSpeciality('unpublish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSpeciality('publish', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSpeciality('trash', $item);
            ...
            if (is_numeric($id) && $id > 0) {
                $item = self::findOne($id);
                self::bulkActionWithSpeciality('restore', $item);
     ...
     * @return void
     */
    public static function bulkActionWithSpeciality($type, $model, $with_childs = true)
    {
        if ($model) {
     ...
     * @return void
     */
    public static function deleteSpeciality($model, $with_childs = true)
    {
        if ($model) {
            ...
            if ($model->delete(false)) {
                $id = $model->id;
                $info = SpecialityInfo::find()->where(['content_id' => $id])->all();
