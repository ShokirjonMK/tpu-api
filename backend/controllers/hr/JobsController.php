...
class JobsController extends BackendController
{
    ...
    public function actionPublished()
    {
        ...
        $where = ['job.status' => 1, 'job.deleted' => 0];
    ...
    public function actionUnpublished()
    {
        ...
        $where = ['job.status' => 0, 'job.deleted' => 0];
    ...
    public function page($type, $where_query, $bulk_actions, $where_in = array())
    {
        ...
        $query = Job::getItems('', $args)->andWhere($where_query);
        ...
        if ($ajax == 'bulk-actions') {
            ...
            $output = Job::ajaxAction($ajax_action, $ajax_item_id, $items);
        ...
        $page_types = Job::getPageTypes($type);
    ...
    public function actionCreate()
    {
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $id = Job::createItem($model, $info, $post_item);
    ...
    public function actionEdit()
    {
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $action = Job::updateItem($model, $info, $post_item);
