...
class DepartmentsController extends BackendController
{
    ...
    public function actionPublished()
    {
        ...
        $where = ['department.status' => 1, 'department.deleted' => 0];
    ...
    public function actionUnpublished()
    {
        ...
        $where = ['department.status' => 0, 'department.deleted' => 0];
    ...
    public function page($type, $where_query, $bulk_actions, $where_in = array())
    {
        ...
        $query = Department::getItems('', $args)->andWhere($where_query);
        ...
        if ($ajax == 'bulk-actions') {
            ...
            $output = Department::ajaxAction($ajax_action, $ajax_item_id, $items);
        ...
        $page_types = Department::getPageTypes($type);
        $listAll = Department::listAll(null, $this->selected_language);
        $listTypes = Department::listTypes();
    ...
    public function actionCreate()
    {
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $id = Department::createItem($model, $info, $post_item);
        ...
        $listTypes = Department::listTypes();
        $listOtherDepartments = Department::listOtherDepartments($model->id, $this->selected_language);
    ...
    public function actionEdit()
    {
        ...
        $item = Department::getItemToEdit($id);
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $action = Department::updateItem($model, $info, $post_item);
