...
class ContentController extends BackendController
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!is_null($type) && $type) {
            ...
            $settings = filter_array($content_types, ['slug' => $type], true);
        }

        if ($settings) {
            $this->settings = $settings;
            $this->url = "/content/{$type}";
            $this->content_type = array_value($settings, 'key');

            Content::$type = $this->content_type;
            Content::$items_with_parent = array_value($settings, 'items_with_parent');
            Content::$slug_generator = array_value($settings, 'slug_generator');
        }
    ...
    public function actionPublished()
    {
        ...
        $where = ['content.status' => 1, 'content.deleted' => 0];
    ...
    public function actionUnpublished()
    {
        ...
        $where = ['content.status' => 0, 'content.deleted' => 0];
    ...
    public function page($type, $where_query, $bulk_actions, $where_in = array())
    {
        ...
        $page_types = Content::getPageTypes($type);
