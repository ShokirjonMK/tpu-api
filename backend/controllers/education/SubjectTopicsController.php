...
use backend\models\SubjectTopic;
 ...
 */
class SubjectTopicsController extends BackendController
{
    public $url = '/education/subject-topics';
    public $settings;
    ...
    public function actionPublished()
    {
        ...
        $where = ['subject_topic.status' => 1, 'subject_topic.deleted' => 0];
    ...
    public function actionUnpublished()
    {
        ...
        $where = ['subject_topic.status' => 0, 'subject_topic.deleted' => 0];
    ...
    public function page($type, $where_query, $bulk_actions, $where_in = array())
    {
        ...
        $query = SubjectTopic::getItems('', $args)->andWhere($where_query);
        ...
        if ($ajax == 'bulk-actions') {
            ...
            $output = SubjectTopic::ajaxAction($ajax_action, $ajax_item_id, $items);
        ...
        $page_types = SubjectTopic::getPageTypes($type);
        $listSubjects = Subject::listAll($this->selected_language);
        $listTypes = TopicType::list();
    ...
    public function actionCreate()
    {
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $id = SubjectTopic::createItem($model, $info, $post_item);
    ...
    public function actionEdit()
    {
        ...
        $item = SubjectTopic::getItemToEdit($id);
        ...
        if ($post_item) {
            ...
            if ($model->load($post_item) && $info->load($post_item)) {
                $action = SubjectTopic::updateItem($model, $info, $post_item);
